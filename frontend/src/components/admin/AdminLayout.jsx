...
export default function AdminLayout() {
  ...
  return (
    <div className="min-h-screen bg-[#FAF2EA] text-charcoal flex overflow-hidden font-sans selection:bg-gold selection:text-white">
      ...
      {/* Sidebar Navigation (Rich Deep Warm Charcoal Shell with Gold Highlighting) */}
      <aside
        className={`fixed md:static inset-y-0 left-0 z-50 w-64 bg-gradient-to-b from-[#3A2618] via-[#2B1B12] to-[#241811] border-r border-gold/30 shadow-2xl text-cream flex flex-col justify-between transition-transform duration-300 transform ${
      ...
      >
        <div>
          ...
          <div className="p-6 border-b border-gold/20 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-2xl bg-gradient-to-tr from-rust via-gold to-gold-glow p-[1px] shadow-gold-glow">
                <div className="w-full h-full bg-[#241811] rounded-2xl flex items-center justify-center">
              ...
              <div>
                <h1 className="font-serif font-bold text-lg text-cream tracking-wider">
                  AURA <span className="text-gold text-xs font-montserrat font-bold">CONTROL</span>
                </h1>
                <span className="text-[9px] uppercase font-montserrat font-bold text-gold/80 tracking-widest block">
                  Cinematic Admin
                </span>
              </div>
            </div>
                ...
                <NavLink
                  ...
                  className={`flex items-center gap-3.5 px-4 py-3 rounded-2xl text-xs font-montserrat font-semibold transition-all duration-200 border ${
                    isActive
                      ...
                      : 'text-cream/80 hover:text-cream hover:bg-gold/10 border-transparent'
                  }`}
                >
                  <Icon className={`w-4 h-4 ${isActive ? 'text-gold' : 'text-cream/60'}`} />
        ...
        <div className="p-4 border-t border-gold/20 space-y-3">
          <div className="p-3 rounded-2xl bg-gold/5 border border-gold/20 text-xs font-sans space-y-1">
          ...
          <motion.button
            whileHover={{ x: -4 }}
            onClick={() => navigate('/')}
            className="w-full py-2.5 px-4 rounded-2xl bg-gold/15 border border-gold/50 text-gold hover:bg-gold/25 text-xs font-montserrat font-bold uppercase tracking-wider transition-all flex items-center justify-center gap-2 shadow-gold-glow-shadow cursor-pointer"
          >
          ...
          <button
            ...
            className="w-full py-2.5 px-4 rounded-2xl bg-rust/15 border border-rust/40 text-cream hover:bg-rust/30 text-xs font-montserrat font-bold uppercase tracking-wider transition-colors flex items-center justify-center gap-2 shadow-sm cursor-pointer"
          >
        ...
        <header className="sticky top-0 z-30 bg-white/90 backdrop-blur-xl border-b border-rust/15 px-6 py-4 flex items-center justify-between shadow-sm">
          <div className="flex items-center gap-4">
            ...
            <div className="hidden sm:block">
              <span className="text-xs sm:text-sm font-montserrat uppercase font-bold text-rust tracking-widest block">
                Studio Management Portal
              </span>
              <span className="text-sm text-charcoal/70 font-serif italic block">
